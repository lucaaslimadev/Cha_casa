    ...
    if (invalidChatIds.length > 0) {
      ...
      return NextResponse.json(
        { 
          ...
          details: `Os Chat IDs devem ser números. IDs inválidos encontrados: ${invalidChatIds.join(', ')}.`
        },
        { status: 500 }
      )
    }

    // Formatar mensagem de confirmação de presença (diferente de presente)
...
_Confirmado em ${new Date().toLocaleString("pt-BR")}_
    `.trim()
    ...
    console.error("Error sending RSVP message:", error)
    return NextResponse.json(
